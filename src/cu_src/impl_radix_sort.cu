...
/*
		int tempa[NUM_KEYS_PER_THREAD] ;
		#pragma unroll
		for (int j = 1 ; j <= WARPSIZE ; j <<= 1) {
			#pragma unroll
			for (int i = 0 ; i < NUM_KEYS_PER_THREAD ; ++i) {
	//			numFlags[i] = warpIncPrefixSum(numFlags[i], laneID, WARPSIZE) ;
				tempa[i] = __shfl_up(numFlags[i],j) ;
				if (laneID >= j) {
					numFlags[i] += tempa[i] ;
				}
			}
...
*/
		...
		for (int i = 0 ; i < NUM_KEYS_PER_THREAD ; ++i) {
			if (globalOffset+i*WARPSIZE < numElements) {
				...
				currentVal[0] = valuesInShr[readLoc] ;
...
void ImplRadixSort::sort(const Camera & camera, std::vector<int> & centroidIDsVec, int * centroidIDs, float * centroidPos) {
	...
	for (int i = 0 ; i < 30 ; i+=RADIXWIDTH) {
		...
		downsweepScan<<<reductionGrid,reductionBlock>>>(keyPtr1, keyPtr2, valPtr1, valPtr2, blockReduceArray, numCentroids, i, numTiles) ;
...
void ImplRadixSort::benchSort(const Camera & camera, std::vector<int> & centroidIDsVec, int * centroidIDs, float * centroidPos, std::vector<float> & times) {
